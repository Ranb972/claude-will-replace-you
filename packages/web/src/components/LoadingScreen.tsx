...
export function LoadingScreen({ visible, onMinimumElapsed }: LoadingScreenProps) {
  ...
  useEffect(() => {
    ...
    const animate = (now: number) => {
      ...
      // Using a piecewise approach for suspense
      let pct: number;
      if (elapsed < 600) {
        // 0-600ms: rush to 40%
        pct = (elapsed / 600) * 40;
      } else if (elapsed < 1400) {
        // 600-1400ms: crawl from 40% to 60% (suspense zone)
        pct = 40 + ((elapsed - 600) / 800) * 20;
      } else if (elapsed < 2000) {
        // 1400-2000ms: accelerate from 60% to 90%
        pct = 60 + ((elapsed - 1400) / 600) * 30;
      } else {
        // 2000ms+: slowly approach 95% (never hits 100 until done)
